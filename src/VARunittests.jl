...
function VARsim(length, typ, coefficients, other_c)
    ...
    typ == "Const and trend" &&
        (other = [fill(1.0, length + burnout) [1:(length + burnout)]])

    y = fill(0.0, length + burnout, series)
    eps = rand(Normal(0, 1 / 100), (length + burnout, series))

    for i in (lags + 1):(size(y)[1])
        y[i, :] =
            mapreduce(
                (x) -> y[i - x, :] * coefficients[(1:series) + (x - 1) * series, :],
                +,
                1:lags,
            ) +
            eps[i, :] +
...
function VARdatarandsim(lags, series, length, typ)
    ...
    typ == "Const and trend" &&
        (other_c = [rand(Uniform(), series) rand(Uniform(), series)])
...
apply(vcat, [coeficients[(1:series) + (x - 1) * series, :] for x in [lags:-1:1]]) - varEstimate(y, lags, "None").C
