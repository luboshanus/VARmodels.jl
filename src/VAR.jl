...
mutable struct varEstimate <: VARRepresentation
    ...
    function varEstimate(data::Matrix{Float64}, lags::Int, typ::String)
        (X, Y, obs, vars, C, Σ, ntyp) = estimateVAR(
            data::Matrix{Float64}, lags::Int, typ::String
        )
        return new(
            lags,
            typ,
...
struct varEstimateShrink <: VARRepresentation
    ...
    function varEstimateShrink(data::Matrix{Float64}, lags::Int, typ::String, λ)
        (X, Y, obs, vars, C, Σ, ntyp) = estimateVARShrinkage(
            data::Matrix{Float64}, lags::Int, typ::String, λ
        )
        return new(
            lags,
            typ,
    ...
    if (a.typ == "Const") || (a.typ == "Trend")
        ...
        println(
            ...
            mapslices(
                ...
                mapslices(
                    (x) -> join(x, "  "),
                    map((x) -> format(x; precision=2, signed=true), a.C[1:1, :]);
                    dims=[2],
                );
                dims=[1],
            )[
    ...
    elseif a.typ == "Const and trend"
        ...
        println(
            ...
            mapslices(
                ...
                mapslices(
                    (x) -> join(x, "  "),
                    map((x) -> format(x; precision=2, signed=true), a.C[e:2, :]);
                    dims=[2],
                );
                dims=[1],
            )[
    ...
    for i in 1:(a.lags)
        ...
        println(
            ...
            mapslices(
                ...
                mapslices(
                    (x) -> join(x, "  "),
                    map((x) -> format(x; precision=2, signed=true), Clag(a, i));
                    dims=[2],
                );
                dims=[1],
            )[
...
function simulateVAR(estimate::VARRepresentation, N, burnout=1000)
    ...
    return simulateVAR(
        estimate.Σ, [Clag(estimate, i) for i in 1:(estimate.lags)], exogen, N, burnout
    )
end
